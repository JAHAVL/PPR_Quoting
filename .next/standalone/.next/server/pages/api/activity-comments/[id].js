...
const getActivityComments = ()=>{
    try {
        ...
        if (!fs__WEBPACK_IMPORTED_MODULE_0___default().existsSync(activityCommentsDbPath)) {
            ...
            fs__WEBPACK_IMPORTED_MODULE_0___default().writeFileSync(activityCommentsDbPath, JSON.stringify([], null, 2));
            return [];
        }
...
const saveActivityComments = (comments)=>{
    try {
        fs__WEBPACK_IMPORTED_MODULE_0___default().writeFileSync(activityCommentsDbPath, JSON.stringify(comments, null, 2));
        return true;
    } catch (error) {
        ...
        try {
            ...
            if (!saveResult) {
                ...
                return res.status(500).json({
                    error: "Failed to save updated comments"
                });
            }
            ...
            return res.status(500).json({
                error: "Failed to delete activity comment"
            });
        }
    } else {
        res.setHeader("Allow", [
...
var __webpack_exports__ = (__webpack_exec__(4547));
